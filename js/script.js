...
submitBtn.addEventListener("click", (e) => {
  ...
  document.getElementById(
    "bt-counted-price-b"
  ).textContent = `${priceAmountBasic} €`;
  document.getElementById("bt-counted-price-s").textContent = `${formatPrice(
    priceAmountSenior
  )} €`;

  document.getElementById("bt-total-output").textContent = `${formatPrice(
    totalAmount
  )} €`;
});
...
function updateBreakdown() {
  ...
  const ticketCountBasic = Number(
    document.getElementById("bt-quantity-b").value
  );
  const ticketCountSenior = Number(
    document.getElementById("bt-quantity-s").value
  );
